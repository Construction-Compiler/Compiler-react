...
export const isOperator = (word) => {
    ...
    for (i in operators) {
        if (operators[i].operator === word) {
            console.log("IT IS OPERATOR: "+operators[i].operator+ "AND WORD: "+ word+ " OPER CLASS: "+operators[i].CP)
            return operators[i].CP
        }
    }
...
export const isFloatConstant = (word) => {
    console.log('in verify float')
    console.log(word)
    // var a = /^([+|-]?\d+)?.\d*$/.test(word)
    var a=/^[+-]?([0-9]*[.])?[0-9]+$/.test(word)
    return a
}
